	...
	class Redux_Validation_Color extends Redux_Validate {
		...
		public function validate() {
			...
			if ( ! in_array( strlen( $test ), array( 3, 6 ), true ) ) {
				...
				$this->field['msg'] = $this->field['msg'] ?? sprintf( esc_html__( 'Invalid HTML color code %1$s. Please enter a valid code. No value was saved.', 'dina-kala' ), '<code>' . $this->value . '</code>' );
			...
			if ( $sanitized_value !== $this->value ) {
				// translators: %1$s: sanitized value.  %2$s: Old value.
				$this->field['msg'] = $this->field['msg'] ?? sprintf( esc_html__( 'Sanitized value and saved as %1$s instead of %2$s.', 'dina-kala' ), '<code>' . $sanitized_value . '</code>', '<code>' . $this->value . '</code>' );
