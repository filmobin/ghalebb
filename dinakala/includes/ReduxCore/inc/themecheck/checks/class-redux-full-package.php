...
class Redux_Full_Package implements themecheck {
	...
	public function check( $php_files, $css_files, $other_files ): bool {
		...
		if ( $redux ) {
			...
			$blacklist = array(
				'codestyles'                       => esc_html__( 'Redux Code Styles', 'dina-kala' ),
				'class-redux-framework-plugin.php' => esc_html__( 'Redux Plugin File', 'dina-kala' ),
				'.travis.yml'                      => esc_html__( 'CI Testing File', 'dina-kala' ),
			);
			...
			if ( ! empty( $errors ) ) {
				$error  = '<span class="tc-lead tc-required">REQUIRED</span> ' . esc_html__( 'It appears that you have embedded the full Redux package inside your theme. You need only embed the', 'dina-kala' ) . ' <strong>redux-core</strong> ' . esc_html__( 'folder. Embedding anything else will get your rejected from theme submission. Suspected Redux package file(s):', 'dina-kala' );
				$error .= '<ol>';
