	...
	class Redux_Extension_Color_Scheme extends Redux_Extension_Abstract {
		...
		public function parse_ajax() {
			if ( isset( $_REQUEST['nonce'] ) && isset( $_REQUEST['opt_name'] ) && wp_verify_nonce( sanitize_key( wp_unslash( $_REQUEST['nonce'] ) ), 'redux_' . sanitize_text_field( wp_unslash( $_REQUEST['opt_name'] ) ) . '_color_schemes' ) ) {
				...
				wp_die( esc_html__( 'Invalid Security Credentials.  Please reload the page and try again.', 'dina-kala' ) );
			}
		}
		...
		private function import_schemes() {
			if ( isset( $_REQUEST['content'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
				...
				if ( is_null( $content ) ) {
					$result = array(
						'result' => false,
						'data'   => esc_html__( 'Import unsuccessful! Malformed JSON data detected.', 'dina-kala' ),
					);
				...
				if ( isset( $content['Default']['color_scheme_name'] ) ) {
					...
					if ( true === Redux_Core::$filesystem->execute( 'put_contents', $import_file, $param_array ) ) {
						$result = array(
							...
							'data'   => sprintf( esc_html__( 'Import successful! Click %s to refresh.', 'dina-kala' ), '<strong>' . esc_html__( 'OK', 'dina-kala' ) . '</strong>' ),
						);
					} else {
						$result = array(
							'result' => false,
							'data'   => esc_html__( 'Import unsuccessful! File permission error: Could not write import data to server.', 'dina-kala' ),
						);
					}
				} else {
					$result = array(
						'result' => false,
						'data'   => esc_html__( 'Import unsuccessful! The selected file is not a valid color scheme file.', 'dina-kala' ),
					);
				}
			} else {
				$result = array(
					'result' => false,
					'data'   => esc_html__( 'Import unsuccessful! No data detected in the import file.', 'dina-kala' ),
				);
			}
