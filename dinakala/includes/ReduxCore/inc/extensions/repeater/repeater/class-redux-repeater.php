	...
	class Redux_Repeater extends Redux_Field {
		...
		public function render() {
			...
			if ( isset( $this->value['redux_repeater_data'] ) && is_array( $this->value['redux_repeater_data'] ) && ! empty( $this->value['redux_repeater_data'] ) ) {
				...
				foreach ( $repeaters as $repeater ) {
					...
					if ( is_array( $repeater['title'] ) ) {
						$repeater['title'] = esc_html__( 'Title', 'dina-kala' );
					}
					...
					foreach ( $this->field['fields'] as $field ) {
						...
						if ( in_array( $field['type'], $unallowed, true ) ) {
							echo esc_html__( 'The', 'dina-kala' ) . ' <code>' . esc_html( $field['type'] ) . '</code> ' . esc_html__( 'field is not supported within the Repeater field.', 'dina-kala' );
						} else {
					...
					if ( ! isset( $this->field['static'] ) && empty( $this->field['static'] ) ) {
						echo '<a href="javascript:void(0);" class="button deletion redux-warning-primary redux-repeaters-remove">' . esc_html__( 'Delete', 'dina-kala' ) . ' ' . esc_html( $this->field['item_name'] ) . '</a>';
					}
			...
			if ( 0 === $x || ( isset( $this->field['static'] ) && ( $x - 1 ) < $this->field['static'] ) ) {
				...
				while ( $loop > 0 ) {
					...
					if ( ! isset( $this->field['static'] ) && empty( $this->field['static'] ) ) {
						echo '<a href="javascript:void(0);" class="button deletion redux-repeaters-remove">' . esc_html__( 'Delete', 'dina-kala' ) . ' ' . esc_html( $this->field['item_name'] ) . '</a>';
					}
			...
			if ( ! isset( $this->field['static'] ) && empty( $this->field['static'] ) ) {
				...
				echo '<a href="javascript:void(0);" class="button redux-repeaters-add button-primary' . esc_attr( $disabled ) . '" data-name="' . esc_attr( $this->parent->args['opt_name'] . $this->repeater_values ) . '[title][]">' . esc_html__( 'Add', 'dina-kala' ) . ' ' . esc_html( $this->field['item_name'] ) . '</a><br/>';
			}
		}
		...
		public function localize( array $field, string $value = '' ) {
			...
			if ( ! empty( $field['fields'] ) ) {
				...
				$var = array(
					'html'     => $var . '<a href="javascript:void(0);" class="button deletion redux-repeaters-remove">' . esc_html__( 'Delete', 'dina-kala' ) . '</a>',
					'count'    => count( $value ),
					'sortable' => true,
					'limit'    => '',
