	...
	class Redux_Icon_Select extends Redux_Field {
						...
						if ( false === stripos( $sub_arr['url'], '//' ) ) {
							...
							if ( is_wp_error( $to_parse ) ) {
								$data = array(
									'parent'  => $this->parent,
									'type'    => 'error',
									'msg'     => 'Icon Select: ' . esc_html__( 'Error retrieving stylesheet ', 'dina-kala' ) . ' "' . $sub_arr['url'] . '". (' . esc_html( $to_parse->get_error_code() ) . ' - ' . esc_html( $to_parse->get_error_message() ) . ')',
								...
								$data = array(
									'parent'  => $this->parent,
									'type'    => 'error',
									'msg'     => 'Icon Select: ' . esc_html__( 'Error retrieving stylesheet ', 'dina-kala' ) . ' "' . $sub_arr['url'] . '". (' . wp_remote_retrieve_response_code( $to_parse ) . ' - ' . esc_html( wp_remote_retrieve_response_message( $to_parse ) ) . ')',
									'id'      => 'Icon_Select_notice_',
									'dismiss' => false,
								);
		...
		public function set_defaults() {
			$defaults = array(
				...
				'class'            => '',
				'button_title'     => esc_html__( 'Add Icon', 'dina-kala' ),
				'remove_title'     => esc_html__( 'Remove Icon', 'dina-kala' ),
				'elusive'          => true,
				'fontawesome'      => true,
				'dashicons'        => true,
				'exclude_icons'    => '',
					...
					<div class="redux-modal-table-cell">
						...
						<div class="redux-modal-inner">
							<div class="redux-modal-title">
								<?php esc_html_e( 'Add Icon', 'dina-kala' ); ?>
							...
							<div class="redux-modal-header">
								...
								<label for="redux-icon-search">
									<input type="text" placeholder="<?php esc_html_e( 'Search...', 'dina-kala' ); ?>" class="redux-icon-search" id="redux-icon-search"/>
								</label>
