...
if ( ! class_exists( 'Redux_Multi_Media' ) ) {
	...
	class Redux_Multi_Media extends Redux_Field {
		...
		public function render() {
			...
			$button_text    = $this->field['labels']['button'] ?? esc_html__( 'Add or Upload File(s)', 'dina-kala' );
			...
			if ( $this->value && is_array( $this->value ) ) {
				...
				foreach ( $this->value as $id => $url ) {
					...
					if ( $this->is_valid_img_ext( $url ) ) {
						...
						$html .= '<a href="#" class="redux_remove_file_button">' . esc_html__( 'Remove Image', 'dina-kala' ) . '</a>';
						$html .= '</p>';
						$html .= $id_input;
						$html .= '</li>';
						...
						// Add file to array.
						$html  = '<li>';
						$html .= esc_html__( 'File: ', 'dina-kala' );
						$html .= '<strong>' . $title . '</strong>&nbsp;&nbsp;&nbsp;';
						$html .= '(<a href="' . $url . '" target="_blank" rel="external">' . esc_html__( 'Download', 'dina-kala' ) . '</a> / <a href="#" class="redux_remove_file_button">' . __( 'Remove', 'dina-kala' ) . '</a>)';
						$html .= $id_input;
						$html .= '</li>';
		...
		public function enqueue() {
			...
			$upload_file    = $this->field['labels']['upload_file'] ?? esc_html__( 'Select File(s)', 'dina-kala' );
			$remove_image   = $this->field['labels']['remove_image'] ?? esc_html__( 'Remove Image', 'dina-kala' );
			$remove_file    = $this->field['labels']['remove_file'] ?? esc_html__( 'Remove', 'dina-kala' );
			$file_label     = $this->field['labels']['file'] ?? esc_html__( 'File: ', 'dina-kala' );
			$download_label = $this->field['labels']['download'] ?? esc_html__( 'Download', 'dina-kala' );
			$media_title    = $this->field['labels']['title'] ?? 'Title';

			// translators: %s: Filename.
			$dup_warn = $this->field['labels']['duplicate'] ?? esc_html__( '%s already exists in your file queue.', 'dina-kala' );

			// translators: %s: Upload limit.
			$max_warn = $this->field['labels']['max_limit'] ?? esc_html__( 'Maximum upload limit of %s reached/exceeded.', 'dina-kala' );
