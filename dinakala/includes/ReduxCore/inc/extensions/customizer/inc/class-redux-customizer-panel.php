	...
	protected function render_fallback() {
		...
		<li id="accordion-panel-<?php echo esc_attr( $this->id ); ?>" class="<?php echo esc_attr( $classes ); ?>"
			...
			<h3 class="accordion-section-title" tabindex="0">
				...
				?>
				<span class="screen-reader-text"><?php esc_html_e( 'Press return or enter to open this panel', 'dina-kala' ); ?></span>
			</h3>
		...
		">
			<div class="accordion-section-title" tabindex="0">
				<span class="preview-notice">
					...
					<?php printf( esc_html__( 'You are customizing', 'dina-kala' ) . ' %s', '<strong class="panel-title">' . esc_html( $this->title ) . '</strong>' ); ?>
				</span>
			</div>
	...
	protected function content_template() {
		...
		<li
			...
			<button class="customize-panel-back" tabindex="-1">
				<span class="screen-reader-text"><?php esc_attr_e( 'Back', 'dina-kala' ); ?></span>
			</button>
			<div class="accordion-section-title">
				<span class="preview-notice">
					<?php /* translators: %s is the site/panel title in the Customizer */ ?>
					<?php printf( esc_html__( 'You are customizing', 'dina-kala' ) . ' %s', '<strong class="panel-title">{{ data.title }}</strong>' ); ?>
				</span>
				<# if ( data.description ) { #>
				<button
					...
					tabindex="0"
					aria-expanded="false">
					<span class="screen-reader-text"><?php esc_attr_e( 'Help', 'dina-kala' ); ?></span>
				</button>
				<# } #>
			</div>
			<# if ( data.description ) { #>
		...
		<li id="accordion-panel-{{ data.id }}"
			...
			<h3 class="accordion-section-title">
				<?php if ( version_compare( $this->wp_ver, '6.7', '<' ) ) { ?>
					...
					<span class="accordion-trigger screen-reader-text"><?php echo esc_html__( 'Press return or enter to open this panel', 'dina-kala' ); ?></span>
				<?php } else { ?>
