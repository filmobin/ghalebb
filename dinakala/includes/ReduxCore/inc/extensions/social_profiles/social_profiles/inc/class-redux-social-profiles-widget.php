	...
	class Redux_Social_Widget_Display extends WP_Widget {
		...
		public function __construct( $params ) {
			...
			$widget_ops = array(
				...
				'description' => __( 'Display social media links', 'dina-kala' ),
		...
		public function form( $instance ) {
			$defaults = array(
				'title' => esc_html__( 'Social', 'dina-kala' ),
			);
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>">
					<?php esc_html_e( 'Title', 'dina-kala' ); ?>
					:
					<input
						class="widefat"
				...
				<label for="redux-social-icons-info">
					...
					printf( esc_html__( 'Control which icons are displayed and their urls on the %1$sssettings page%2$s', 'dina-kala' ), '<a href="' . esc_url( admin_url( 'admin.php?page=' . esc_attr( $slug ) . '&tab=' . esc_attr( $tab ) ) ) . '">', '</a>' );
					?>
				</label>
			</p>
			<?php
