	...
	class Redux_Users {
		...
		public static function set_profile( string $opt_name = '', array $profile = array() ) {
			...
			if ( ! empty( $opt_name ) && is_array( $profile ) && ! empty( $profile ) ) {
				...
				self::$errors[ $opt_name ]['profile']['empty'] = esc_html__( 'Unable to create a profile due an empty profile array or the profile variable passed was not an array.', 'dina-kala' );
			}
		}
