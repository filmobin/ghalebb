		...
		public function render() {
			...
			foreach ( $mode_arr as $mode ) {
				...
				if ( ! isset( $this->field['transparent'] ) || false !== $this->field['transparent'] ) {
					...
					echo '<input type="checkbox" class="checkbox color-transparency ' . esc_attr( $this->field['class'] ) . '" id="' . esc_attr( $this->field['id'] ) . '-' . esc_attr( $mode ) . '-transparency" data-id="' . esc_attr( $this->field['id'] ) . '-' . esc_attr( $mode ) . '" value="1"' . esc_html( $trans_checked ) . '> ' . esc_html__( 'Transparent', 'dina-kala' );
					echo '</label>';
				}
