	...
	class Redux_Page_Render extends Redux_Class {
		...
		public function load_page() {
			...
			if ( true === $this->show_hints ) {
				...
				$dismiss = 'true';
				$s       = esc_html__( 'Enable', 'dina-kala' );

				// Values for disabling hints.
				if ( 'true' === $hint_status ) {
					$dismiss = 'false';
					$s       = esc_html__( 'Disable', 'dina-kala' );
				}
				...
				$event = esc_html__( 'moving the mouse over', 'dina-kala' );
				if ( 'click' === $core->args['hints']['tip_effect']['show']['event'] ) {
					$event = esc_html__( 'clicking', 'dina-kala' );
				}

				// Construct message.
				// translators: %1$s: Mouse action.  %2$s: Hint status.
				$msg = sprintf( esc_html__( 'Hints are tooltips that popup when %1$s the hint icon, offering addition information about the field in which they appear.  They can be %2$s by using the link below.', 'dina-kala' ), $event, Redux_Core::strtolower( $s ) ) . '<br/><br/>' . $url;

				// Construct hint tab.
				$tab = array(
					'id'      => 'redux-hint-tab',
					'title'   => esc_html__( 'Hints', 'dina-kala' ),
					'content' => '<p>' . $msg . '</p>',
				);
			...
			if ( '' !== $core->args['help_sidebar'] ) {
				...
				$screen->set_help_sidebar( '<p><strong>Redux Framework</strong><br/><br/>' . esc_html__( 'Hint Tooltip Preferences', 'dina-kala' ) . '</p>' );
			}
				...
				if ( $class_file ) {
					...
					if ( ! class_exists( $field_class ) ) {
						if ( file_exists( $class_file ) ) {
							...
							printf( esc_html__( 'Field %1$s could not be displayed. Field type %2$s was not found.', 'dina-kala' ), '<code>' . esc_attr( $field['id'] ) . '</code>', '<code>' . esc_attr( $field['type'] ) . '</code>' );
						}
					}
				}
		...
		private function get_default_output_string( array $field ): string {
			...
			if ( ! empty( $default_output ) ) {
				$default_output = esc_html__( 'Default', 'dina-kala' ) . ': ' . substr( $default_output, 0, - 2 );
			}
		...
		public function section_menu( $k, array $section, string $suffix = '', array $sections = array() ): string {
			...
			if ( isset( $section['type'] ) && 'divide' === $section['type'] ) {
				...
				$extra_icon         = $subsections ? '<span class="extraIconSubsections"><i class="fal fa-chevron-down">&nbsp;</i></span>' : '';
