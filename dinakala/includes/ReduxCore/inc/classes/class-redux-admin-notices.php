							...
							if ( true === $notice['dismiss'] ) {
								...
								if ( ! get_user_meta( $userid, 'ignore_' . $notice['id'] ) ) {
									...
									if ( version_compare( $wp_version, '4.2', '>' ) ) {
										...
										echo '<div ' . esc_html( $add_style ) . ' class="' . esc_attr( $notice['type'] ) . ' notice is-dismissable"><p>' . wp_kses_post( $notice['msg'] ) . '&nbsp;&nbsp;<a href="?dismiss=true&amp;id=' . esc_attr( $css_id ) . '">' . esc_html__( 'Dismiss', 'dina-kala' ) . '</a>.</p></div>';
									}
								}
							} else {
								// Standard notice.
