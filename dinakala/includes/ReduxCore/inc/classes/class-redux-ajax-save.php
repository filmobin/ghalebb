	...
	class Redux_AJAX_Save extends Redux_Class {
		...
		public function save() {
			...
			if ( ! isset( $_REQUEST['nonce'] ) || ( ! wp_verify_nonce( sanitize_key( wp_unslash( $_REQUEST['nonce'] ) ), 'redux_ajax_nonce' . $this->args['opt_name'] ) ) ) {
				echo wp_json_encode(
					array(
						'status' => esc_html__( 'Invalid security credential.  Please reload the page and try again.', 'dina-kala' ),
						'action' => '',
					)
				);
				die();
			...
			if ( ! Redux_Helpers::current_user_can( $core->args['page_permissions'] ) ) {
				echo wp_json_encode(
					array(
						'status' => esc_html__( 'Invalid user capability.  Please reload the page and try again.', 'dina-kala' ),
						'action' => '',
					)
				);
				die();
			...
			if ( isset( $_POST['opt_name'] ) && ! empty( $_POST['opt_name'] ) && isset( $_POST['data'] ) && ! empty( $_POST['data'] ) ) {
				...
				if ( ! empty( $redux->args['opt_name'] ) ) {
					...
					if ( ! empty( $values ) ) {
						...
						echo wp_json_encode(
							array(
								'status' => esc_html__( 'Your panel has no fields. Nothing to save.', 'dina-kala' ),
							)
						);
						die();
					}
