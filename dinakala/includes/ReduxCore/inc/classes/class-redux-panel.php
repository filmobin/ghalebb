...
if ( ! class_exists( 'Redux_Panel', false ) ) {
	...
	class Redux_Panel {
		...
		private function panel_template() {
			...
			// Do we support JS?
			echo '<noscript><div class="no-js">' . esc_html__( 'Warning- This options panel will not work properly without javascript!', 'dina-kala' ) . '</div></noscript>';
		...
		public function notification_bar() {
			if ( isset( $this->parent->transients['last_save_mode'] ) ) {
				...
				if ( 'import' === $this->parent->transients['last_save_mode'] ) {
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					echo '<strong>' . esc_html( apply_filters( "redux-imported-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings Imported!', 'dina-kala' ) ) ) . '</strong>';
					echo '</div>';
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					echo '<strong>' . esc_html( apply_filters( "redux-defaults-text-{$this->parent->args['opt_name']}", esc_html__( 'All Defaults Restored!', 'dina-kala' ) ) ) . '</strong>';
					echo '</div>';
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					echo '<strong>' . esc_html( apply_filters( "redux-defaults-section-text-{$this->parent->args['opt_name']}", esc_html__( 'Section Defaults Restored!', 'dina-kala' ) ) ) . '</strong>';
					echo '</div>';
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					echo '<strong>' . esc_html( apply_filters( "redux-saved-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings Saved!', 'dina-kala' ) ) ) . '</strong>';
					echo '</div>';
				}
			...
			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			echo '<strong>' . esc_html( apply_filters( "redux-changed-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings have changed, you should save them!', 'dina-kala' ) ) ) . '</strong>';
			echo '</div>';
			...
			echo '<strong>';
			echo '<span></span> ' . esc_html__( 'error(s) were found!', 'dina-kala' );
			echo '</strong>';
			echo '</div>';
			...
			echo '<strong>';
			echo '<span></span> ' . esc_html__( 'warning(s) were found!', 'dina-kala' );
			echo '</strong>';
			echo '</div>';
		}
		...
		public function get_template( string $file ) {
			...
			if ( file_exists( $path ) ) {
				if ( is_readable( $path ) ) {
					...
					echo '<div class="error"><p>' . sprintf( esc_html__( 'Redux Panel Template %1$s cannot be read. Please check the permissions for this file.', 'dina-kala' ), '<code>' . esc_html( $path ) . '</code>' ) . '</p></div>';
				}
			} elseif ( file_exists( Redux_Core::$dir . 'templates/panel/' . $file ) ) {
					require Redux_Core::$dir . 'templates/panel/' . $file;
			} else {
				// translators: %1$s: template path.
				echo '<div class="error"><p>' . sprintf( esc_html__( 'Redux Panel Template %1$s does not exist. Please reinstall Redux to replace this file.', 'dina-kala' ), '<code>' . esc_html( $path ) . '</code>' ) . '</p></div>';
			}
		}
				...
				if ( $developer_theme_file ) {
					...
					if ( $core_version && $developer_version && version_compare( $developer_version, $core_version, '<' ) ) {
						...
						<div id="message" class="error redux-message" style="display:block!important">
							<p>
								<strong><?php esc_html_e( 'Your panel has bundled copies of Redux Framework template files that are outdated!', 'dina-kala' ); ?></strong>&nbsp;&nbsp;<?php esc_html_e( 'Please ask the author of this theme to update them as functionality issues could arise.', 'dina-kala' ); ?>
							</p>
						</div>
						<?php
