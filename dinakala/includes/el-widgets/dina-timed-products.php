...
class Dina_Timed_Products extends Widget_Base {

	public function get_name() {
		return 'dina-timed-products';
	}
	
	public function get_title() {
		return __( 'Timed discount products (Dinakala)', 'dina-kala' );
	}
	
	public function get_icon() {
		return 'fal fa-alarm-clock';
	}

	public function get_help_url() {
		return 'https://i-design.ir/docs/dinakala/?p=1015';
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'sort_rand',
			[
				'label'        => __( 'Show products randomly', 'dina-kala' ),
				'type'         => Controls_Manager::SWITCHER,
				'label_on'     => __( 'On', 'dina-kala' ),
				'label_off'    => __( 'Off', 'dina-kala' ),
				'return_value' => 'yes',
	...
	protected function render() {
		...
		$sort_rand = $settings['sort_rand'];

		$args = array (
			'post_status'            => 'publish',
			'post_type'              => 'product',
			'post__in'               => array_merge( array( 0 ), dina_scheduled_sale_products() ),
							...
							if ( ! empty( $date ) && ( $product->is_purchasable() || $product -> is_type( 'external' ) ) && $product->is_in_stock() && !show_login_price() ) {
								...
								if ( $product->is_on_sale() ) { ?>
									<span class="counter-con">
										<span class="prodcounter sale-count-text" data-datediff="<?php echo $datediff; ?>" data-format="<?php echo $settings['sale_count_format']; ?>" data-countdown="<?php echo $sale_price_date; ?>" data-dir="<?php dina_dir() ?>"></span>
									</span>
								<?php } 
							}
						} ?>
