...
class SearchWP_Live_Search_Form extends SearchWP_Live_Search {
	...
	function assets() {
		// styles
		//wp_enqueue_style( SA_URI . '/assets/styles/style.css', null, $this->version );
		...
		if ( $debug ) {
			wp_register_script(
				...
				SA_URI . 'assets/javascript/build/searchwp-live-search.js',
				array( 'jquery' ),
				$this->version,
				true
			);
			...
			wp_register_script(
				'swp-live-search-client',
				SA_URI . 'assets/javascript/build/searchwp-live-search.min.js',
				array( 'jquery' ),
				$this->version,
				true
			);
		...
		$params = array(
			...
			'config'                => $this->configs,
			'msg_no_config_found'   => __( 'No valid SearchWP Live Search configuration found!', 'dina-kala' ),
			'aria_instructions'     => __( 'When autocomplete results are available use up and down arrows to review and enter to go to the desired page. Touch device users, explore by touch or with swipe gestures.' , 'dina-kala' ),
		);
