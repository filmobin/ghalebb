...
include_once( SA_DIR . '/includes/class-template.php' );
	...
	function search() {
		...
		if ( isset( $_REQUEST['swpquery'] ) && ! empty( $_REQUEST['swpquery'] ) ) {
			...
			if ( class_exists( 'SearchWP' ) ) {
				...
				$args = array(
					'post_status'   => 'publish', // we're limiting to a pre-set array of post IDs
					'post__in'      => $posts
				);
			} else {
			...
			if ( dina_opt( 'search_others' ) ) {
				$args['post_type'][] = 'post';
				$args['post_type'][] = 'page';
			}

			if ( dina_opt( 'hide_outstock_result' ) ) {
			...
			} elseif ( dina_opt( 'ajax_search_stock' ) ) {
			...
			$args['posts_per_page'] = dina_opt( 'ajax_count' );
